...
class LogicalReferenceValidationTest {
  ...
  void addCartItemRejectsMealSlotOutsideCurrentFamily() {
    ...
    CartApplicationServiceImpl service = new CartApplicationServiceImpl(
        mapper, new ExpectedMealTimePolicy(), commands,
        new ObjectMapper().findAndRegisterModules());

    assertThrows(BusinessException.class, () -> service.mutateItem(USER,
        new CartMutationRequest(null, 0L, "request-1", 21L, 1, null)));
