<template>
  <div class="detail-page">
    <div class="detail-topbar"><button class="ghost-button" type="button" @click="router.push('/platform-dish-templates')">返回列表</button><button class="primary-button" type="button" :disabled="saving || !form" @click="save">{{ saving ? '保存中...' : '保存模板' }}</button></div>
    ...
    <template v-else-if="form && detail">
      ...
      <form class="editor-layout" @submit.prevent="save">
        <section class="editor-panel"><header><h3>基本信息</h3><span>派生状态由服务端保存后重新计算</span></header><div class="form-grid">
          ...
          <label class="form-field"><span>分类 ID</span><input v-model="form.categoryId" type="number" min="1" required /></label>
...
const loading = ref(true); const saving = ref(false); const assetBusy = ref(false);
...
async function load() { loading.value = true; try { prepare(await getAdminDishTemplate(route.params.templateId)); } finally { loading.value = false; } }
...
function addStep() { form.value.cookingSteps.push({ itemId: `step-new-${Date.now()}`, title: '', content: '', durationSeconds: '', temperatureText: '', heatLevel: '', componentTemplateId: '' }); }
function removeStep(index) { form.value.cookingSteps.splice(index, 1); }
async function save() { const snapshot = buildTemplateSnapshot(form.value); const validation = validateTemplateSnapshot(snapshot); if (validation) { notify(validation, 'error'); return; } const { sortOrder, ...editable } = snapshot; saving.value = true; try { await updateAdminDishTemplate(detail.value.templateId, { ...editable, schemaVersion: 2, expectedVersion: detail.value.version }); notify('模板已保存，完整状态已重新计算', 'success'); await load(); } finally { saving.value = false; } }
...
async function promote(asset) { const values = promotionForms[asset.assetId]; if (!values.author || !values.sourceUrl || !values.license) { notify('请补全作者、来源页面和授权说明', 'error'); return; } assetBusy.value = true; try { await promoteDishTemplateImage(detail.value.templateId, { internalAssetId: asset.assetId, expectedVersion: detail.value.version, ...values }); notify('图片已发布', 'success'); await load(); } finally { assetBusy.value = false; } }
async function reject(asset) { const reason = await promptAction({ title: '驳回内部图片', label: '驳回原因', required: true, danger: true, confirmText: '确认驳回' }); if (!reason) return; assetBusy.value = true; try { await rejectDishTemplateImage(detail.value.templateId, asset.assetId, reason); notify('图片已驳回', 'success'); await load(); } finally { assetBusy.value = false; } }
