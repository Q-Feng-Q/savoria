...
云托管容器只运行 Spring Boot。不要把 MySQL 放进云托管容器：服务允许缩容到零并扩容到多个实例，容器本地磁盘不能作为生产数据库持久化层。
...
```text
[cloudrun-entrypoint] database=<地址>/family_kitchen user=<账号> port=8080
[cloudrun-entrypoint] starting Spring Boot
