...
- [ ] 将运行层改为包含 MySQL 8、Java 17、Nginx 和 tini 的 Ubuntu 镜像。
- [ ] 把所有持久化目录统一到 `/data`。
- [ ] 添加面向小型服务器的 MySQL 默认参数。
- [ ] 添加同时检查 MySQL 与 HTTP 代理链路的健康检查。
...
- [ ] 校验必填环境变量和安全字符。
- [ ] 首次启动初始化 MySQL 数据目录。
- [ ] 创建数据库与业务账号并等待 MySQL 就绪。
- [ ] 启动并监控 Spring Boot 与 Nginx。
- [ ] 实现信号转发和失败联动退出。
...
- [ ] 将单镜像章节改为单容器内置 MySQL 的真实命令。
- [ ] 说明 HTTPS 接入、持久化、备份、恢复和升级步骤。
- [ ] 运行静态验证脚本，检查 Dockerfile、入口脚本、挂载路径和必需文档。
