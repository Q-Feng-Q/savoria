...
**Architecture:** 新增独立 Java 17 离线生成工具读取外部固定提交，不在应用运行时联网；工具输出规范化清单、质量报告、私有图片清单和确定性 SQL 数据段。正式发布通过新的 Flyway 前向迁移应用数据，避免修改已执行迁移。Spring Boot 继续采用传统 MVC，Service 直接调用 MyBatis Mapper；模板市场只暴露采购与价格均就绪的成品菜，平台端可维护全部菜谱和组件。
...
- `backend/src/main/resources/db/migration/V13__sync_complete_cooklikehoc_recipe_catalog.sql`: 本功能新增表、字段和完整同步数据；保留原 `V4/V5/V6` 作为不可变历史迁移。
