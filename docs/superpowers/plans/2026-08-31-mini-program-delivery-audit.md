...
module.exports = Object.freeze({
  ...
  pages: new Map([
    ['pages/auth/entry/index', { domain: 'auth', async: true, form: true, refresh: false, tenantScope: 'public' }],
...
```powershell
git add frontend/tests/fixtures/delivery-audit-manifest.js frontend/tests/delivery-audit-inventory.test.js
...
For each factory export in the 11 service modules, add an invocation fixture and expected normalized request. Dynamic identifiers use `{id}` in the backend JSON and concrete sentinel values in Node tests.
...
Capture calls through injected `request`/`upload`, then assert method, path, encoded query, omitted `undefined`, JSON body, and response unwrapping. Add negative fixtures for missing IDs, invalid dates/enums, and `null` query values.
...
Create JSON entries with `operation`, `method`, `route`, `scope`, `validRequest`, `invalidRequests`, `successStatus`, and `errorStatuses`. Generate the values from the reviewed manifest, but check in the deterministic JSON so Java tests do not execute frontend code.

- [ ] **Step 4: Write and run the failing Spring route test**

Use `RequestMappingHandlerMapping` to normalize registered Spring routes and assert every JSON method/route pair exists exactly once. For representative GET/query, PUT/JSON, POST/JSON, DELETE, date, enum, and dynamic-ID cases, use MockMvc to verify binding reaches a controller rather than returning route-level 404 or binding 400 for the valid request. Invalid requests must return the documented 400/401/403/404 family and stable application error envelope.
...
- [ ] **Step 3: Add the database isolation contract**

Inspect the resolved datasource URL/profile used by tests. Allow only `jdbc:h2:mem:` or a Testcontainers-managed disposable JDBC URL/database name. Explicitly reject the configured business host/database and require destructive integration tests to carry the container marker/profile.
...
```powershell
git add backend/src/test/java/com/familykitchen/contract backend/src/test/java/com/familykitchen/database backend/src/main/java backend/src/main/resources/mapper
git add -f docs/superpowers/audits/2026-08-31-mini-program-delivery-findings.md
...
For every `async` page, require a loading state and a retryable error state. For every `list` page, require a distinct empty state. For every `form` or mutation page, require pending/disabled protection and failure recovery. Render representative null payloads and assert the UI never contains literal `null`, `undefined`, `NaN`, raw enum codes, internal IDs presented without labels, or retired personal-wallet/meal-slot copy.
...
Assert action controls use the custom component, have an accessible label, disabled/pending behavior, and an 88rpx/44px-equivalent tap target. For 320×568, 375×667, 390×844, and 430×932 with 0/34px safe areas, inspect shared CSS contracts for shrink-safe grids, wrapping, scroll clearance, and horizontal-overflow prevention. Add fixtures for doubled text, maximum phone/amount, missing image, and failed image fallback.
...
Use the installed current Developer Tools project at `frontend/project.config.json`. Trigger a full compile (CLI if configured, otherwise GUI), record tool/library versions, and require zero WXML/WXSS errors across 37 pages. If unavailable, leave this gate explicitly open rather than reporting success.
...
Expected: at least 348 baseline tests plus all new tests; 0 failures. Record total, duration, and any skipped/disabled test with reason.

- [ ] **Step 2: Run the full backend suite**

Run: `D:\develop\apache-maven-3.9.9\bin\mvn.cmd test`

Expected: at least 285 baseline tests plus all new tests; 0 failures. Record every one of the baseline 12 or changed skip set and whether critical MySQL gates executed.

- [ ] **Step 3: Exercise the core journey matrix**

Verify with test harnesses and available local runtime: login/register/recovery; account/role switch; family create/join/edit/member transfer/address; merchant profile/family/menu/dish/import/recommendation; ordering category/full list/detail/shared cart/expected time; family wallet/order submission; merchant order transition/purchase. Every journey must map to passing automated evidence or a named manual environment check.

- [ ] **Step 4: Resolve or explicitly gate every finding**

No P0/P1/P2 finding may remain open. P3 can remain only with user-visible justification and must not contradict the approved visual system. Critical Docker/MySQL or WeChat compiler checks that cannot run are release gates, not “passed” items.
...
```powershell
git add -f docs/superpowers/audits/2026-08-31-mini-program-delivery-findings.md docs/superpowers/audits/2026-08-31-mini-program-delivery-verification.md
...
- [ ] **Step 2: Re-read main worktree changes**

In `D:\develop\Project\Kitchen`, run `git status --short` and `git diff --` for the existing cart files. Record the paths and confirm they remain untouched by the audit branch unless an audit finding explicitly required coordinated changes.

- [ ] **Step 3: Integrate commits safely**

Use a non-destructive merge or ordered cherry-picks. If an audit commit overlaps a user-modified cart file, stop automatic integration, preserve both diffs, and resolve only with explicit evidence; never reset or discard the user's version.

- [ ] **Step 4: Re-run full verification in the integrated main worktree**

Run frontend and backend full suites again. Expected: 0 failures and counts no lower than the audit worktree. Report any environment-gated checks separately.
